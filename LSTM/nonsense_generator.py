...
def train(opt):
    ...
    # print(sorted(list(enumerate(data)), key=lambda x:x[1])[:10])
    # tmp = []
    # for s in data:
    #     if len(s) > 2:
    #         tmp.append(s)
    ...
    dataloader = torch.utils.data.DataLoader(data, batch_size=opt.batch_size, drop_last=True, shuffle=False, num_workers=10)
    ...
    # checkpoint = './checkpoints/english_my2.pth'
    ...
    for epoch in range(1, 11):
        ...
        for step, bz_data in tqdm(enumerate(dataloader)):
            try:
...
def eval(net, ix2word, word2ix, prefix_words):
    poem = beam_generate(net, ix2word, word2ix, prefix_words=prefix_words, beam_width=10, qsize_min=500)
...
def infer(prefix_words, beam_size, qsize):
    ...
    net = nn.DataParallel(net)
    checkpoint = './checkpoints/english_10.pth'
